...
_libcudnn.cudnnSetTensorNdDescriptor.argtypes = [ctypes.c_void_p, ctypes.c_int,
                                                 ctypes.c_int, 
                                                 ctypes.POINTER(ctypes.c_int),
                                                 ctypes.POINTER(ctypes.c_int)]
def cudnnSetTensorNdDescriptor(tensorDesc, dataType, nbDims, dimA, strideA):
    """
    Initialize a previously created generic Tensor object.

    This function initializes a previously created generic Tensor 
    descriptor object.
    ...
    assert len(dimA) == nbDims and len(strideA) == nbDims
    array_type = ctypes.c_int * nbDims 
    status = _libcudnn.cudnnSetTensorNdDescriptor(tensorDesc, dataType, nbDims,
        array_type(*dimA), array_type(*strideA))
    cudnnCheckStatus(status)
...
_libcudnn.cudnnSetFilterNdDescriptor.argtypes = [ctypes.c_void_p, ctypes.c_int,
                                                 ctypes.c_int,
                                                 ctypes.POINTER(ctypes.c_int)]
def cudnnSetFilterNdDescriptor(filterDesc, dataType, nbDims, filterDimA):
    """"
    Initialize a filter descriptor.
    ...
    assert len(filterDimA) == nbDims
    array_type = ctypes.c_int * nbDims 
    status = _libcudnn.cudnnSetFilterNdDescriptor(filterDesc, dataType, nbDims, 
        array_type(*filterDimA))
    cudnnCheckStatus(status)
...
_libcudnn.cudnnSetConvolutionNdDescriptor.argtypes = [ctypes.c_void_p, 
                                                    ctypes.c_int,
                                                    ctypes.POINTER(ctypes.c_int), 
                                                    ctypes.POINTER(ctypes.c_int), 
                                                    ctypes.POINTER(ctypes.c_int), 
...
def cudnnSetConvolutionNdDescriptor(convDesc, arrayLength, padA, filterStrideA,
    upscaleA, mode):
    """"
    Initialize a convolution descriptor.
    ...
    mode : cudnnConvolutionMode
        Select between CUDNN_CONVOLUTION or CUDNN_CROSS_CORRELATION.
    """
    assert len(padA) == arrayLength and len(filterStrideA) == arrayLength and \
        len(upscaleA) == arrayLength
    array_type = ctypes.c_int * arrayLength 
    status = _libcudnn.cudnnSetConvolutionNdDescriptor(convDesc, arrayLength,
        array_type(*padA), array_type(*filterStrideA), array_type(*upscaleA),
        mode)
    cudnnCheckStatus(status)
    
_libcudnn.cudnnGetConvolution2dForwardOutputDim.restype = ctypes.POINTER(ctypes.c_int)
_libcudnn.cudnnGetConvolution2dForwardOutputDim.argtypes = [ctypes.c_void_p, 
    ctypes.c_void_p, ctypes.c_void_p, ctypes.c_int, 
    ctypes.POINTER(ctypes.c_int)]
def cudnnGetConvolutionNdForwardOutputDim(convDesc, inputTensorDesc, 
    filterDesc, nbDims):
    """"
    Return the dimensions of the output tensor given a convolution descriptor.
    ...
    w : int
        Width of each output feature map.
    """
    array_types = ctypes.c_int * nbDims#ctypes.byref(
    tensorOutputDimA = [0] * nbDims
    tensorOutputDimA = array_types(*tensorOutputDimA)
    status = _libcudnn.cudnnGetConvolutionNdForwardOutputDim(convDesc, inputTensorDesc,
                                                 filterDesc, 
                                                 nbDims,
                                                 tensorOutputDimA)
    tensorOutputDimAsList =  [tensorOutputDimA[i] for i in range(nbDims)]
    cudnnCheckStatus(status)
    return tensorOutputDimAsList
